...
/**
 * Contract to control the release of KSWAP.
 */
contract TreasuryVester is Ownable, ReentrancyGuard {
    ...
    // Amount of KSWAP required to start distributing denominated in wei
    // Should be 512 million KSWAP
    ...
    // KSWAP Distribution plan:
    // According to the Kalyswap Litepaper, we initially will distribute
    // 175342.465 KSWAP per day. Vesting period will be 24 hours: 86400 seconds.
    ...
    /**
     * Enable distribution. A sufficient amount of KSWAP >= startingBalance must be transferred
    ...
    function startVesting() external onlyOwner {
        ...
        require(IERC20(kswap).balanceOf(address(this)) >= startingBalance, 'TreasuryVester::startVesting: incorrect KSWAP supply');
    ...
    /**
     * Sets the recipient of the vested distributions. In the initial Kalyswap scheme, this
    ...
    /**
     * Vest the next KSWAP allocation. Requires vestingCliff seconds in between calls. KSWAP will
    ...
    function claim() external nonReentrant returns (uint) {
        ...
        IERC20(kswap).safeTransfer(recipient, vestingAmount);
