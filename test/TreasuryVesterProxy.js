...
const KSWAP_TOTAL_SUPPLY = BigNumber.from('538000000000000000000000000');
...
describe('TreasuryVesterProxy', function() {
    ...
    beforeEach(async function() {
        this.kswap = await this.KSWAP.deploy(this.owner.address);
        await this.kswap.deployed();

        this.treasuryVester = await this.TreasuryVester.deploy(this.kswap.address);
        await this.treasuryVester.deployed();

        this.communityTreasury = await this.CommunityTreasury.deploy(this.kswap.address);
        await this.communityTreasury.deployed();

        this.miniChef = await this.MiniChef.deploy(this.kswap.address, this.owner.address);
        ...
        await this.kswap.transfer(this.treasuryVester.address, TREASURY_VESTER_INITIAL_AMOUNT);
        await this.kswap.transfer(this.communityTreasury.address, TREASURY_INITIAL_AMOUNT);
    ...
    describe('TreasuryVester Constructor', function() {
        it('kswap default', async function() {
            expect((await this.treasuryVester.kswap())).to.equal(this.kswap.address);
        });
    ...
    describe('TreasuryVesterProxy Constructor', function() {
        it('kswap default', async function() {
            expect((await this.treasuryVesterProxy.kswap())).to.equal(this.kswap.address);
        });
    ...
    describe('claimAndDistribute', function() {
        ...
        it('1st claim diverts to treasury and chef', async function() {
            ...
            const treasuryBalance = await this.kswap.balanceOf(this.communityTreasury.address);
            const chefBalance = await this.kswap.balanceOf(this.miniChef.address);

            // Claim
            await this.treasuryVesterProxy.claimAndDistribute();

            // Ensure diversion to treasury
            expect(await this.kswap.balanceOf(this.communityTreasury.address)).to.equal(treasuryBalance.add(STARTING_DIVERSION));
            // Ensure remainder to chef
            expect(await this.kswap.balanceOf(this.miniChef.address)).to.equal(chefBalance.add(STARTING_AMOUNT).sub(STARTING_DIVERSION));
        });
        ...
        it('31th claim increases diversion to treasury', async function() {
            ...
            // Ensure diversion to treasury
            expect(await this.kswap.balanceOf(this.communityTreasury.address)).to.equal(treasuryBalance.add(expectedDiversion));
            // Ensure remainder to chef
            expect(await this.kswap.balanceOf(this.miniChef.address)).to.equal(chefBalance.add(STARTING_AMOUNT).sub(expectedDiversion));
        });
    ...
    describe('Integration test', function() {
        ...
        xit('Works as expected', async function() {
            ...
            const initialCirculatingSupply = KSWAP_TOTAL_SUPPLY
                .sub(await this.kswap.balanceOf(this.treasuryVester.address))
                .sub(await this.kswap.balanceOf(this.communityTreasury.address)); // Avoid double counting

            // Claim loops
            for (let i = 1; i <= DAYS_TO_PROJECT; i++) {
                await this.treasuryVesterProxy.claimAndDistribute();
                await increaseBlocktime(86400);
                const [kswapTreasury, kswapChef] = await Promise.all([
                    this.kswap.balanceOf(this.communityTreasury.address),
                    this.kswap.balanceOf(this.miniChef.address)
                ]);
                const circulatingSupply = initialCirculatingSupply.add(kswapTreasury).add(kswapChef);
                console.log(`Day ${i} :: ${kswapTreasury / 10 ** 18} / ${kswapChef / 10 ** 18} / ${circulatingSupply / 10 ** 18}`);
                expect(kswapTreasury.lte(TREASURY_TARGET_BALANCE)).to.be.true;
                expect(circulatingSupply.lte(DESIRED_TOTAL_SUPPLY)).to.be.true;
                if (i === DAYS_TO_PROJECT) {
                    expect(kswapTreasury.eq(TREASURY_TARGET_BALANCE)).to.be.true;
