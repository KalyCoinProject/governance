...
contract LiquidityPoolManager is Ownable, ReentrancyGuard {
    ...
    function calculateReturns() public {
        ...
        for (uint i = 0; i < klcPairs.length(); i++) {
            uint pairLiquidity = getKlcLiquidity(klcPairs.at(i));
        ...
        if (kswapPairs.length() > 0) {
            ...
            for (uint i = 0; i < kswapPairs.length(); i++) {
                uint pairLiquidity = getKswapLiquidity(kswapPairs.at(i), conversionRatio);
                distribution[i + klcPairs.length()] = pairLiquidity;
                totalLiquidity = SafeMath.add(totalLiquidity, pairLiquidity);
            }
        }
