...
describe('Governance', function () {
    before(async function () {
        ...
        this.Wklc = await ethers.getContractFactory("Kswap");
        this.AltCoin = await ethers.getContractFactory("Kswap");
        ...
        this.Factory = await ethers.getContractFactory("KalyswapFactory");
    ...
    beforeEach(async function () {
        ...
        this.kswapHandle2 = await this.kswap.connect(this.addr2);
        this.kswapHandle3 = await this.kswap.connect(this.addr3);
        ...
        // GovernorAlpha
        this.governor = await this.Governor.deploy(this.timelock.address, this.kswap.address, OWNER_ADDRESS);
        ...
        // WKLC
        this.wklc = await this.Wklc.deploy(OWNER_ADDRESS);
        ...
        this.lpManager = await this.LpManager.deploy(this.wklc.address, this.kswap.address,
        ...
        // Setup mocks
        await this.mockPairKlc.givenMethodReturnAddress(token0, this.wklc.address);
    ...
    describe("Timelock Proposals", function () {
        ...
        it('Add whitelisted Pool', async function () {
            ...
            await expect(this.lpManager.addWhitelistedPool(this.mockPairKlc.address,1)).to.be.revertedWith('Ownable: caller is not the owner');

            const target = this.lpManager.address;
            const value = 0;
            const sig = 'addWhitelistedPool(address,uint256)';
            const callData = ethers.utils.defaultAbiCoder.encode(['address','uint256'], [this.mockPairKlc.address, 1]);
        ...
        it('Remove Whitelisted Pool', async function () {
            ...
            // Set LpManager owner
            await this.lpManager.transferOwnership(this.timelock.address);
            await expect(this.lpManager.removeWhitelistedPool(this.mockPairKlc.address)).to.be.revertedWith('Ownable: caller is not the owner');

            const target = this.lpManager.address;
            const value = 0;
            const sig = 'removeWhitelistedPool(address)';
            const callData = ethers.utils.defaultAbiCoder.encode(['address'], [this.mockPairKlc.address]);
        ...
        it('Spend From Community Treasury', async function () {
            ...
            expect(await this.kswap.balanceOf(UNPRIVILEGED_ADDRESS)).to.equal(0);
            ...
            await ethers.provider.send("evm_setNextBlockTimestamp", [updateTime]);
            await this.timelock.executeTransaction(target, value, sig, callData, startTime + DELAY);
            expect(await this.kswap.balanceOf(UNPRIVILEGED_ADDRESS)).to.equal(transferAmount);
    ...
    describe("Governor Proposals", function () {
        it('Change Admin', async function () {
            ...
            // Transfer KSWAP
            await this.kswap.transfer(this.addr2.address, this.proposalThreshold);
            await this.kswap.transfer(this.addr3.address, this.voteThreshold);

            // Delegate KSWAP so addresses can vote, must delegate to themselves
            await this.kswapHandle2.delegate(this.addr2.address);
            await this.kswapHandle3.delegate(this.addr3.address);
            await this.kswap.delegate(OWNER_ADDRESS);
        ...
        it('Add whitelisted Pool', async function () {
            ...
            const value = 0;
            const sig = 'addWhitelistedPool(address,uint256)';
            const callData = ethers.utils.defaultAbiCoder.encode(['address', 'uint256'], [this.mockPairKlc.address, 5]);
